        ...
        for picking in self.picking_ids:
            if not picking.transsmart_confirmed:
                try:
                    ...
                    failed_pickings.append(picking.name and str(picking.name) or '')
                    continue
        if failed_pickings:
            raise Warning(_("Creating a transsmart document failed for the following picking(s): %s" % (failed_pickings)))
