...
# pylint: disable=too-many-arguments, missing-docstring, invalid-name
import getpass
import json
import logging
import requests
...
from openerp import _, api, exceptions, fields, models, registry
...
class DeliveryWebService(models.Model):
    ...
    def get_current_service(self):
        ...
        if not current:
            raise exceptions.ValidationError(
                _("No cofiguration defined for current os user %s.") % instance_user
            )
    ...
    def post(self, endpoint, payload=None):
        ...
        response = requests.post(
            request_url,
            json=payload and clean_empty(payload) or None,
    ...
    def create_or_write_transsmart(self, model, vals):
        ...
        record = model.with_context(active_test=False).search(
            [('transsmart_code', '=', vals['transsmart_code'])]
        )
        if not record:
            model.create(vals)
        else:
